function [VOI, STATES, ALGEBRAIC, CONSTANTS] = Paci_VoltageClamp_iKs(Vclamp,iKs_scaling_factors)

   [VOI, STATES, ALGEBRAIC, CONSTANTS] = solveModel(Vclamp,iKs_scaling_factors);
end
...
function [VOI, STATES, ALGEBRAIC, CONSTANTS] = solveModel(Vclamp,iKs_scaling_factors)
    % Create ALGEBRAIC of correct size
    global algebraicVariableCount;  algebraicVariableCount = getAlgebraicVariableCount();
    % Initialise constants and state variables
    [INIT_STATES, CONSTANTS] = initConsts(iKs_scaling_factors);
    ...
    [VOI, STATES] = ode15s(@(VOI, STATES)computeRates(VOI, STATES, CONSTANTS, Vclamp, iKs_scaling_factors), tspan, INIT_STATES, options);

    % Compute algebraic variables
    [RATES, ALGEBRAIC] = computeRates(VOI, STATES, CONSTANTS, Vclamp, iKs_scaling_factors);
    ALGEBRAIC = computeAlgebraic(ALGEBRAIC, CONSTANTS, STATES, VOI, iKs_scaling_factors);
...
function [STATES, CONSTANTS] = initConsts(iKs_scaling_factors)
    VOI = 0; CONSTANTS = []; STATES = []; ALGEBRAIC = [];
    %STATES(:,1) = -0.0743340057623841;
    CONSTANTS(:,1) = 1.45454e-11;
    CONSTANTS(:,2) = 0;
    ...
    CONSTANTS(:,31) = 0.002;
    CONSTANTS(:,32) = 29.8667;
    STATES(:,11) = 0.00778547011240132;
    STATES(:,12) = 0.432162576531617;
    CONSTANTS(:,33) = 0.025;
    CONSTANTS(:,34) = 2.3;
    CONSTANTS(:,35) = iKs_scaling_factors(1)*2.041;
...
function [RATES, ALGEBRAIC] = computeRates(VOI, STATES, CONSTANTS, Vclamp, iKs_scaling_factors)
...
STATES(1) = (Vhold - Vbase)*VOI(end).^n./(tstart.^n + VOI(end).^n) + Vbase;
    ...
    ALGEBRAIC(:,48) = iKs_scaling_factors(2)*(( 1.00000.*( ALGEBRAIC(:,25).*ALGEBRAIC(:,39)))./1000.00);
...
function ALGEBRAIC = computeAlgebraic(ALGEBRAIC, CONSTANTS, STATES, VOI, iKs_scaling_factors)
    statesSize = size(STATES);
    statesColumnCount = statesSize(2);
    if ( statesColumnCount == 1)
        STATES = STATES';
