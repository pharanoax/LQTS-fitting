function [VOI, STATES, ALGEBRAIC, CONSTANTS] = Paci_VoltageClamp_iKr(Vclamp,iKr_scaling_factors)

   [VOI, STATES, ALGEBRAIC, CONSTANTS] = solveModel(Vclamp,iKr_scaling_factors);
end
...
function [VOI, STATES, ALGEBRAIC, CONSTANTS] = solveModel(Vclamp,iKr_scaling_factors)
    % Create ALGEBRAIC of correct size
    global algebraicVariableCount;  algebraicVariableCount = getAlgebraicVariableCount();
    % Initialise constants and state variables
    [INIT_STATES, CONSTANTS] = initConsts(iKr_scaling_factors);
    ...
    [VOI, STATES] = ode15s(@(VOI, STATES)computeRates(VOI, STATES, CONSTANTS, Vclamp, iKr_scaling_factors), tspan, INIT_STATES, options);

    % Compute algebraic variables
    [RATES, ALGEBRAIC] = computeRates(VOI, STATES, CONSTANTS, Vclamp, iKr_scaling_factors);
    ALGEBRAIC = computeAlgebraic(ALGEBRAIC, CONSTANTS, STATES, VOI, iKr_scaling_factors);
...
function [STATES, CONSTANTS] = initConsts(iKr_scaling_factors)
    VOI = 0; CONSTANTS = []; STATES = []; ALGEBRAIC = [];
    %STATES(:,1) = -0.0743340057623841;
    CONSTANTS(:,1) = 2.92044e-11;
    CONSTANTS(:,2) = 0;
    CONSTANTS(:,3) = 0; % i_stim_Start in component stim_mode (second)
    CONSTANTS(:,4) = 800; % i_stim_End in component stim_mode (second)
    CONSTANTS(:,5) = 5.5e-10;
    CONSTANTS(:,6) = 60; % i_stim_frequency in component stim_mode (per_second)
    CONSTANTS(:,7) = 0.005; % i_stim_PulseDuration in component stim_mode (second)
    CONSTANTS(:,8) = 0;
    CONSTANTS(:,9) = 0;
    CONSTANTS(:,10) = 0;
    CONSTANTS(:,11) = 0;
    ...
    CONSTANTS(:,66) = 60.0000./CONSTANTS(:,6); % i_stim_Period in component stim_mode (second)
...
function [RATES, ALGEBRAIC] = computeRates(VOI, STATES, CONSTANTS, Vclamp, iKr_scaling_factors)
    global algebraicVariableCount;
    statesSize = size(STATES);
    statesColumnCount = statesSize(2);
    
%     if VOI < 2
%         STATES(1)= -0.04;%-0.0743340057623841;
%     elseif 2 <= VOI <5
%         a = 0.0743340057623841/(5-2);
%         STATES(1)= a*VOI(end) - 5*a;
%     else
%         STATES(1) = 0;
%     end
Vbase = -0.065; % mV; base voltage
Vhold = Vclamp;%-0.07; % mV; holding voltage for clamping
tstart = 2; % s; starting time for clamp
n = 20; % Cooperativity; make larger to increase steepness of curve

STATES(1) = (Vhold - Vbase)*VOI(end).^n./(tstart.^n + VOI(end).^n) + Vbase;
    ...
    ALGEBRAIC(:,46) = iKr_scaling_factors(2)*(( 1.00000.*( ALGEBRAIC(:,23).*ALGEBRAIC(:,37)))./1000.00);
    ...
    ALGEBRAIC(:,47) = iKr_scaling_factors(3)*(( 1.00000.*( ALGEBRAIC(:,24).*ALGEBRAIC(:,38)))./1000.00);
...
function ALGEBRAIC = computeAlgebraic(ALGEBRAIC, CONSTANTS, STATES, VOI, iKr_scaling_factors)
    statesSize = size(STATES);
    statesColumnCount = statesSize(2);
    if ( statesColumnCount == 1)
        STATES = STATES';
