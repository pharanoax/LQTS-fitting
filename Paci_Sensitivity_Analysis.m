...
function [VOI, STATES, ALGEBRAIC, CONSTANTS] = Paci_Sensitivity_Analysis(scaling_factors)
    % This is the "main function".  In Matlab, things work best if you rename this function to match the filename.
   [VOI, STATES, ALGEBRAIC, CONSTANTS] = solveModel(scaling_factors);
end
...
function [VOI, STATES, ALGEBRAIC, CONSTANTS] = solveModel(scaling_factors)
    % Create ALGEBRAIC of correct size
    global algebraicVariableCount;  algebraicVariableCount = getAlgebraicVariableCount();
    % Initialise constants and state variables
    [INIT_STATES, CONSTANTS] = initConsts(scaling_factors);
    ...
    [VOI, STATES] = ode15s(@(VOI, STATES)computeRates(VOI, STATES, CONSTANTS,scaling_factors), tspan, INIT_STATES, options);

    % Compute algebraic variables
    [RATES, ALGEBRAIC] = computeRates(VOI, STATES, CONSTANTS,scaling_factors);
    ALGEBRAIC = computeAlgebraic(ALGEBRAIC, CONSTANTS, STATES, VOI,scaling_factors);
end
...
function [STATES, CONSTANTS] = initConsts(scaling_factors)
    ...
    CONSTANTS(:,27) = 5.4;
    CONSTANTS(:,28) = 0.03;
    CONSTANTS(:,29) = scaling_factors(13)*3671.2302;
    STATES(:,4) = 0.102953468725004;
    STATES(:,5) = 0.786926637881461;
    STATES(:,6) = 0.253943221774722;
    CONSTANTS(:,30) = scaling_factors(1)*8.635702e-5;
    STATES(:,7) = 8.96088425225182e-5;
    STATES(:,8) = 0.970411811263976;
    STATES(:,9) = 0.999965815466749;
    STATES(:,10) = 0.998925296531804;
    CONSTANTS(:,31) = 0.002;
    CONSTANTS(:,32) = scaling_factors(5)*29.8667;
    STATES(:,11) = 0.00778547011240132;
    STATES(:,12) = 0.432162576531617;
    CONSTANTS(:,33) = 0.025;
    CONSTANTS(:,34) = 2.3;
    CONSTANTS(:,35) = scaling_factors(8)*2.041;
    STATES(:,13) = 0.0322944866983666;
    CONSTANTS(:,36) = scaling_factors(10)*28.1492;
    CONSTANTS(:,37) = scaling_factors(17)*30.10312;
    CONSTANTS(:,38) = -0.017;
    STATES(:,14) = 0.100615100568753;
    CONSTANTS(:,39) = 0.9;
    CONSTANTS(:,40) = 0.69264;
    CONSTANTS(:,41) = 1;
    CONSTANTS(:,42) = 40;
    CONSTANTS(:,43) = scaling_factors(11)*1.841424;
    CONSTANTS(:,44) = scaling_factors(12)*4900;
...
function [RATES, ALGEBRAIC] = computeRates(VOI, STATES, CONSTANTS, scaling_factors)
    ...
    ALGEBRAIC(:,21) = scaling_factors(4)*(( ( 600.000.*exp( - power( STATES(:,1).*1000.00+25.0000, 2.00000)./170.000)+(31.0000./(1.00000+exp((25.0000 -  STATES(:,1).*1000.00)./10.0000))+16.0000./(1.00000+exp((30.0000+ STATES(:,1).*1000.00)./10.0000)))).*CONSTANTS(:,70))./1000.00);
    RATES(:,9) = (ALGEBRAIC(:,6) - STATES(:,9))./ALGEBRAIC(:,21);
    ALGEBRAIC(:,12) = 1.00000./(1.00000+exp(( STATES(:,1).*1000.00+77.8500)./5.00000));
    ALGEBRAIC(:,27) = scaling_factors(13)*((1900.00./(1.00000+exp(( STATES(:,1).*1000.00+15.0000)./10.0000)))./1000.00);
    ...
    ALGEBRAIC(:,35) = scaling_factors(3)*(( (20.0000+( 1102.50.*exp( - power(power( STATES(:,1).*1000.00+27.0000, 2.00000)./15.0000, 2.00000))+(200.000./(1.00000+exp((13.0000 -  STATES(:,1).*1000.00)./10.0000))+180.000./(1.00000+exp((30.0000+ STATES(:,1).*1000.00)./10.0000))))).*ALGEBRAIC(:,20))./1000.00);
    ...
    ALGEBRAIC(:,42) = scaling_factors(14)* (piecewise({STATES(:,1)< - 0.0400000, 1.50000./( (ALGEBRAIC(:,17)+ALGEBRAIC(:,32)).*1000.00) }, 2.54200./1000.00));
    ...
    ALGEBRAIC(:,43) = scaling_factors(15)* (7.00000./( (ALGEBRAIC(:,18)+ALGEBRAIC(:,33)).*1000.00));
    ...
    ALGEBRAIC(:,46) = scaling_factors(6)*(( 1.00000.*( ALGEBRAIC(:,23).*ALGEBRAIC(:,37)))./1000.00);
    ...
    ALGEBRAIC(:,47) = scaling_factors(7)*(( 1.00000.*( ALGEBRAIC(:,24).*ALGEBRAIC(:,38)))./1000.00);
    ...
    ALGEBRAIC(:,48) = scaling_factors(9)*(( 1.00000.*( ALGEBRAIC(:,25).*ALGEBRAIC(:,39)))./1000.00);
    ...
    ALGEBRAIC(:,50) = scaling_factors(2)*(( ( ALGEBRAIC(:,19).*ALGEBRAIC(:,34)+ALGEBRAIC(:,44)).*1.00000)./1000.00);
...
function ALGEBRAIC = computeAlgebraic(ALGEBRAIC, CONSTANTS, STATES, VOI,scaling_factors)
    statesSize = size(STATES);
    statesColumnCount = statesSize(2);
    if ( statesColumnCount == 1)
        STATES = STATES';
    ...
    ALGEBRAIC(:,16) = 1.00000./(1.00000+exp((  - STATES(:,1).*1000.00 - 60.0000)./5.00000));
    ALGEBRAIC(:,31) = 0.100000./(1.00000+exp(( STATES(:,1).*1000.00+35.0000)./5.00000))+0.100000./(1.00000+exp(( STATES(:,1).*1000.00 - 50.0000)./200.000));
    ALGEBRAIC(:,41) = scaling_factors(16)*(( 1.00000.*( ALGEBRAIC(:,16).*ALGEBRAIC(:,31)))./1000.00);
    ALGEBRAIC(:,2) = 1.00000./power((1.00000+exp(( STATES(:,1).*1000.00+72.1000)./5.70000)), 1.0 ./ 2);
    ALGEBRAIC(:,17) = piecewise({STATES(:,1)< - 0.0400000,  0.0570000.*exp( - ( STATES(:,1).*1000.00+80.0000)./6.80000) }, 0.000000);
    ALGEBRAIC(:,32) = piecewise({STATES(:,1)< - 0.0400000,  2.70000.*exp( 0.0790000.*( STATES(:,1).*1000.00))+ 3.10000.*( power(10.0000, 5.00000).*exp( 0.348500.*( STATES(:,1).*1000.00))) }, 0.770000./( 0.130000.*(1.00000+exp(( STATES(:,1).*1000.00+10.6600)./ - 11.1000))));
