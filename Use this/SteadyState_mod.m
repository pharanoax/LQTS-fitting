function [SS_VOI, SS_STATES] = SteadyState_mod( t1, ScalingFactors )
...
while ~SS % While steady state has not been reached
    ...
    assert(SStime < 1200,'The data does not reach steady state')
    
    % Run the model for the current guess of the length of time
    [SS_VOI, SS_STATES, ~, ~] = Paci_SS_mod(SStime, ScalingFactors);
