function [VOI, STATES, ALGEBRAIC, CONSTANTS] = Paci_VoltageClamp(Vclamp,scaling_factors)

   [VOI, STATES, ALGEBRAIC, CONSTANTS] = solveModel(Vclamp,scaling_factors);
end
...
function [VOI, STATES, ALGEBRAIC, CONSTANTS] = solveModel(Vclamp,scaling_factors)
    ...
    [VOI, STATES] = ode15s(@(VOI, STATES)computeRates(VOI, STATES, CONSTANTS, Vclamp), tspan, INIT_STATES, options);

    % Compute algebraic variables
    [RATES, ALGEBRAIC] = computeRates(VOI, STATES, CONSTANTS, Vclamp);
...
function [STATES, CONSTANTS] = initConsts(scaling_factors)
    ...
    CONSTANTS(:,31) = 0.002;
    CONSTANTS(:,32) = scaling_factors(2)*29.8667;
    STATES(:,11) = 0.00778547011240132;
    STATES(:,12) = 0.432162576531617;
    CONSTANTS(:,33) = 0.025;
    CONSTANTS(:,34) = 2.3;
    CONSTANTS(:,35) = scaling_factors(1)*2.041;
...
function [RATES, ALGEBRAIC] = computeRates(VOI, STATES, CONSTANTS, Vclamp)
